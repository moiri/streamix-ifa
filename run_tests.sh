...
if [ -z "$t" ] || [ "$t" == "serial" ]; then
    ...
    ./single_test.sh $v$p-f json -ds ./test/serial6.json
    ./single_test.sh $v$p-f json -ds ./test/serial7.json
fi
if [ -z "$t" ] || [ "$t" == "feedback" ]; then
    ...
    ./single_test.sh $v$p-f json -ds ./test/feedback8.json
fi
if [ -z "$t" ] || [ "$t" == "loop" ]; then
...
if [ -z "$t" ] || [ "$t" == "mult" ]; then
    ...
    ./single_test.sh $v$p-f json -ds -db ./test/mult4.json
fi
